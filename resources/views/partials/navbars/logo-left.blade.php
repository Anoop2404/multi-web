@if(($tenant->type ?? null) === 'sahodaya')
    {{-- Sahodaya-only announcement and secretariat bar. School websites begin at the main navbar. --}}
    @php
        $latestCircular = \App\Support\SahodayaPublicData::latestCirculars($tenant->id, 1)->first();
        $tickerSchoolCount = \App\Support\SahodayaPublicData::memberSchools($tenant->id)->count();
    @endphp
                ...
                @foreach($items as $item)
                    ...
                    @if(!empty($item['children']))
                        <div class="relative shrink-0" @mouseenter="activeSubmenu = '{{ md5($label) }}'" @mouseleave="activeSubmenu = null">
                            ...
                            {{-- :class instead of x-show/x-transition on purpose — Alpine's
                                 transition module intermittently left this panel stuck
                                 invisible (or visible-but-unsized) after activeSubmenu
                                 changed, reproducible on both local and production with no
                                 pattern tied to page or timing. A plain :class toggle onto
                                 always-rendered CSS goes through Alpine's much simpler core
                                 reactivity instead, sidestepping the transition module
                                 entirely; the fade/slide is still handled by the `transition`
                                 utility class, just driven by CSS rather than x-transition. --}}
