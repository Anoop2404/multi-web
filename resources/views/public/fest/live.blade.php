...
<section class="py-8 sm:py-12 px-4 bg-slate-950 text-white min-h-screen" id="fest-live-root" data-live-url="{{ route('tenant.fest.live.data', ['event' => $event->id]) }}" data-event-status="{{ $event->status }}" data-show-overall="{{ ($showOverallStandings ?? true) ? '1' : '0' }}">
    <div class="max-w-[100rem] mx-auto">
        ...
        <div id="now-performing" class="mt-5 p-5 rounded-2xl bg-amber-500/10 border-2 border-amber-500/40 text-center shadow-[0_0_30px_-5px_rgba(245,158,11,0.35)] @if(!$nowPerforming) hidden @endif">
            ...
            @if(!empty($nowPerforming['category_label']) || !empty($nowPerforming['gender_label']))
            <span class="text-amber-300/70 text-xs font-semibold uppercase tracking-wide mt-1 block">{{ collect([$nowPerforming['category_label'] ?? null, $nowPerforming['gender_label'] ?? null])->filter()->implode(' · ') }}</span>
            @endif
...
(function () {
    ...
    function renderNow(p) {
        ...
        const meta = [p.category_label, p.gender_label].filter(Boolean).join(' · ');
        if (meta) html += `<span class="text-amber-300/70 text-xs font-semibold uppercase tracking-wide mt-1 block">${esc(meta)}</span>`;
