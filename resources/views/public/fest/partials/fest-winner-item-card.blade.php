<article class="rounded-2xl bg-slate-900 border border-slate-800 shadow-md overflow-hidden">
    <div class="px-6 py-4 bg-white/5 border-b border-slate-800">
        ...
        <p class="text-base text-amber-300/80 font-semibold">{{ collect([$itemGroup['category_label'] ?? null, $itemGroup['gender_label'] ?? null])->filter()->implode(' · ') }}</p>
        @endif
        @if($itemGroup['head'])<p class="text-base text-white/40">{{ $itemGroup['head'] }}</p>@endif
    </div>
    ...
    <div class="flex flex-wrap">
        ...
        <div class="flex gap-4 p-5 flex-1 min-w-[22rem] border-l border-slate-800 first:border-l-0">
            <div class="shrink-0">
                ...
                <img src="{{ asset('images/fest/medals/rank-'.$winner['position'].'.webp') }}" alt="Position {{ $winner['position'] }}" class="w-20 h-20">
                @else
                <span class="w-20 h-20 rounded-xl bg-amber-500/20 text-amber-300 border border-amber-500/30 flex items-center justify-center text-2xl font-extrabold" aria-label="Position {{ $winner['position'] }}">#{{ $winner['position'] }}</span>
                @endif
            </div>
            <div class="min-w-0 flex-1">
                <div class="grid grid-cols-[repeat(auto-fill,minmax(120px,1fr))] gap-3">
                    ...
                    <div class="flex flex-col items-center gap-1.5 w-28">
                        ...
                        <img src="{{ $member['photo'] }}" alt="" class="w-28 h-28 rounded-xl object-cover object-top border-2 border-slate-700/60 shadow-md shadow-black/30">
                        @else
                        <span class="w-28 h-28 rounded-xl bg-amber-500/15 text-amber-300 flex items-center justify-center font-bold text-2xl border-2 border-slate-700/60 shadow-md shadow-black/30">{{ strtoupper(substr($member['name'] ?? '?', 0, 1)) }}</span>
                        @endif
                        <span class="text-sm font-semibold leading-tight text-white/90 text-center uppercase break-words">{{ $member['name'] ?? '—' }}</span>
                    </div>
                    @endforeach
                    @if($hiddenCount)
                    <div class="flex flex-col items-center gap-1.5 w-28">
                        <span class="w-28 h-28 rounded-xl bg-slate-800/60 border-2 border-dashed border-slate-700 flex items-center justify-center text-slate-300 font-extrabold text-xl">+{{ $hiddenCount }}</span>
                        <span class="text-sm font-semibold leading-tight text-white/50 text-center">more</span>
                    </div>
                    @endif
                </div>
