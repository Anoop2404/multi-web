...
<section class="py-8 sm:py-12 px-4 bg-slate-950 text-white min-h-screen">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="mt-4 flex items-center gap-4 rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
            @if($public['photo'])
            <img src="{{ $public['photo'] }}" @if($public['photo_fallback'] ?? null) data-fallback-src="{{ $public['photo_fallback'] }}" onerror="this.onerror=null;this.src=this.dataset.fallbackSrc" @endif decoding="async" alt="" class="w-16 h-16 rounded-xl object-cover object-top border-2 border-slate-700/60 shadow-md shadow-black/30 shrink-0">
            @else
