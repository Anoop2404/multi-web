        ...
        <div class="sticky top-16 z-20 rounded-2xl border border-slate-800 bg-slate-900/90 backdrop-blur shadow-xl p-3 sm:p-4 mb-9">
            <div class="flex flex-col lg:flex-row lg:items-center gap-3">
                ...
                <div class="flex gap-2 overflow-x-auto pb-1 lg:pb-0" aria-label="Filter events by status">
                    ...
                    <button type="button" data-event-status-filter="{{ $key }}" aria-pressed="{{ $key === 'all' ? 'true' : 'false' }}"
                            class="event-filter whitespace-nowrap rounded-xl border px-3.5 py-2.5 text-xs font-bold transition {{ $key === 'all' ? 'bg-amber-500 border-amber-500 text-slate-950' : 'bg-transparent border-slate-700 text-white/60 hover:border-amber-500/50' }}">
                        {{ $label }}
                    </button>
                    @endforeach
                </div>
            </div>
            <p id="event-filter-summary" class="text-xs text-white/40 mt-2 px-1" aria-live="polite">Showing all {{ $events->count() }} events</p>
        </div>
                ...
                <div class="space-y-7">
                    ...
                    <div data-event-series>
                        ...
                        <div class="grid md:grid-cols-2 gap-4">
                            ...
                            @php
                                $searchText = collect([
                                    $event->title, $event->venue, $event->parentEvent?->title,
                                    $event->sourcePhase?->name, $event->region?->name,
                            ...
                            @endphp
                            <a href="{{ route('tenant.fest.show', $event->id) }}" data-event-card data-search="{{ Str::lower($searchText) }}"
                               class="group flex flex-col min-h-56 p-5 sm:p-6 bg-slate-900/60 border border-slate-800 rounded-2xl hover:border-amber-500/50 hover:bg-slate-900 hover:-translate-y-0.5 transition focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 focus:ring-offset-slate-950">
                                <div class="flex items-start justify-between gap-3">
                                    <div class="min-w-0">
                                        <p class="text-[11px] font-bold uppercase tracking-[0.14em] text-amber-400">{{ ucfirst(str_replace('_', ' ', $event->event_type)) }}</p>
                                        <h3 class="font-bold text-lg leading-snug mt-1 text-white group-hover:text-amber-300 transition">{{ $event->title }}</h3>
                                    </div>
                                    <span class="shrink-0 inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[11px] font-bold {{ $statusClass }}">
                                        @if($event->status === 'ongoing')<span class="relative flex h-1.5 w-1.5" aria-hidden="true"><span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span><span class="relative inline-flex rounded-full h-1.5 w-1.5 bg-emerald-400"></span></span>@endif
                                        {{ ucfirst(str_replace('_', ' ', $event->status)) }}
                                    </span>
                                </div>

                                <dl class="mt-5 grid gap-2 text-sm text-white/60">
                                    <div class="flex gap-2.5"><dt class="sr-only">Date</dt><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" class="w-4 h-4 mt-0.5 shrink-0"><rect x="3" y="5" width="18" height="16" rx="2" stroke-width="2"/><path d="M8 3v4M16 3v4M3 10h18" stroke-width="2"/></svg><dd>{{ $event->event_start?->format('d M Y') ?? 'Date to be announced' }}@if($event->event_start && $event->event_end && !$event->event_end->isSameDay($event->event_start)) – {{ $event->event_end->format('d M Y') }}@endif</dd></div>
                                    @if($event->venue)<div class="flex gap-2.5"><dt class="sr-only">Venue</dt><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" class="w-4 h-4 mt-0.5 shrink-0"><path d="M20 10c0 5-8 11-8 11S4 15 4 10a8 8 0 1 1 16 0Z" stroke-width="2"/><circle cx="12" cy="10" r="2.5" stroke-width="2"/></svg><dd>{{ $event->venue }}</dd></div>@endif
                                    @if($event->sourcePhase || $event->region)
                                    <div class="flex gap-2.5"><dt class="sr-only">Event context</dt><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" class="w-4 h-4 mt-0.5 shrink-0"><path d="M5 4v16M5 7h11l-2.5 4L16 15H5" stroke-width="2" stroke-linejoin="round"/></svg><dd>{{ collect([$event->sourcePhase?->name, $event->region?->name])->filter()->implode(' · ') }}</dd></div>
                                    @endif
                                </dl>

                                <div class="mt-auto pt-5 flex items-center justify-between gap-3">
                                    <div class="flex flex-wrap gap-2 text-xs font-bold">
                                        @if($event->schedule_published)<span class="inline-flex items-center gap-1 rounded-full border border-sky-500/30 bg-sky-500/10 text-sky-300 px-2.5 py-1">📅 Schedule</span>@endif
                                        @if($event->results_published)<span class="inline-flex items-center gap-1 rounded-full border border-amber-500/30 bg-amber-500/10 text-amber-300 px-2.5 py-1">🏆 Results</span>@endif
                                    </div>
...
(() => {
    ...
    const apply = () => {
        ...
        cards.forEach(card => {
            ...
            card.hidden = !(matchesStatus && matchesText);
