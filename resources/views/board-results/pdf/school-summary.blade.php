...
<head>
    ...
    <title>School Performance Summary</title>
    <style>
...
<body>
    <div class="header">
        ...
        @endif
        <h1 class="title">School Performance Summary</h1>
        <h2 class="subtitle">Class {{ $selectedClass }} • Academic Year {{ $academicYear }}</h2>
    ...
    <table>
        <tbody>
            <tr>
                ...
                <td class="value">{{ $result->total_appeared ?? '—' }}</td>
            </tr>
            <tr>
                <th>Total Passed</th>
                <td class="value">{{ $result->pass_count ?? '—' }}</td>
            </tr>
            <tr>
                <th>Pass Percentage</th>
                <td class="value" style="color: #059669;">{{ $result->pass_percent ? number_format($result->pass_percent, 2) . '%' : '—' }}</td>
            </tr>
            <tr>
                <th>Distinctions</th>
                <td class="value">{{ $result->distinctions ?? '—' }}</td>
            </tr>
            <tr>
                <th>First Class</th>
                <td class="value">{{ $result->first_class ?? '—' }}</td>
            </tr>
            <tr>
                <th>Highest Mark</th>
                <td class="value">{{ $result->highest_mark ? number_format($result->highest_mark, 2) . '%' : '—' }}</td>
            </tr>
            <tr>
                <th>Average Mark</th>
                <td class="value">{{ $result->average_mark ? number_format($result->average_mark, 2) . '%' : '—' }}</td>
            </tr>
            <tr>
                <th>Status in Sahodaya</th>
                <td class="value" style="text-transform: uppercase;">{{ $result->status ?? 'Draft' }}</td>
            </tr>
        </tbody>
    </table>
    ...
    <div style="margin-top: 20px;">
        ...
        <p style="font-size: 12px; color: #333; background: #f8fafc; padding: 10px; border: 1px solid #e2e8f0; border-radius: 4px; margin-top: 5px;">
            {{ $result->remarks }}
        </p>
    </div>
    @endif
    ...
    <div class="footer">
        {{ $school->name }} • Board Results • <span class="page-number"></span>
    </div>
</body>
</html>
