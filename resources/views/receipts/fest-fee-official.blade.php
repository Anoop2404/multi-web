...
<head>
    <meta charset="utf-8">
    <title>Fest Fee Receipt {{ $receipt->receipt_number }}</title>
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .header { padding-left: 8px; display: flex; gap: 16px; align-items: flex-start; margin-bottom: 16px; }
        ...
        .items-list { margin: 12px 0 0 8px; font-size: 12px; }
...
<body>
    <div class="receipt">
        ...
        <div class="header">
            <div class="org-info">
                <h1>{{ $sahodaya->name }}</h1>
                <p>Sahodaya Schools Complex · Official Fest Fee Receipt</p>
                <p>Date: {{ $receipt->payment_date?->format('d M Y') ?? now()->format('d M Y') }}</p>
            </div>
        </div>

        <div class="receipt-title">Fest Event Fee Receipt</div>
