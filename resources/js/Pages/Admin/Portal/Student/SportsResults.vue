    ...
    >
        <section class="card mb-4">
            ...
            <ul v-else class="text-sm divide-y">
                <li v-for="(r, i) in results" :key="i" class="py-3">
                    <p class="font-medium">{{ r.event_title }}</p>
                    <p class="text-xs text-slate-600 mt-0.5">{{ r.item_title }}</p>
...
const navItems = computed(() => [
    ...
    { href: `/portal/student/${props.school.id}/sports-results`, label: 'Sports results' },
]);
</script>
