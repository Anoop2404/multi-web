<template>
    <AdminLayout :title="`${program.title} — Winners`">
        <PageHeader :title="`${program.title} — Qualified winners`" :eyebrow="eyebrow"
                    description="Participants promoted to the next level across clusters.">
            <template #actions>
                <a :href="`/admin/state-programs/${program.id}/winners/export`" class="btn-secondary text-sm">Export CSV</a>
                <Link :href="`/admin/state-programs/${program.id}`" class="btn-primary text-sm">← Program</Link>
            </template>
        </PageHeader>
...
const props = defineProps({ program: Object, winners: { type: Array, default: () => [] } });
