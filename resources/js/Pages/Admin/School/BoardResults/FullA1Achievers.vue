...
const academicYearSelectOptions = computed(() => props.academicYearOptions.map((ay) => ({
    value: ay.label,
    label: academicYearOptionLabel(ay),
})));

const streamSelectOptions = computed(() => Object.entries(props.streamOptions).map(([key, label]) => ({
    value: label,
    label,
})));
...
function subjectOptionsForRow(i) {
    const subjectOptions = props.standardSubjects.map((s) => ({
        value: s,
        label: `${props.subjectCodes[s] ? `${s} (${props.subjectCodes[s]})` : s}${isSubjectUsedElsewhere(i, s) ? ' — already added' : ''}`,
    }));
    return [...subjectOptions, { value: '__custom__', label: '+ Custom subject...' }];
}
