<template>
    <SahodayaEventsLayout :title="`${event.title} — Reports`" :sahodaya="sahodaya" :event="event"
        ...
        <ReportHeadItemNavigator :groups="headItemGroups"
                                 ...
                                 :hint="'Pick an item head (Athletics, Chess…), then an item to open filtered reports.'"
                                 empty-heads-text="Sync item heads from the competition hub, then return here.">
...
const pageTitle = computed(() => {
    ...
    return `${props.event.title} — Reports by item head`;
});
...
const headerDescription = computed(() => {
    ...
    return 'Navigate by item head, then pick an item — same flow as competition hub.';
});
</script>
