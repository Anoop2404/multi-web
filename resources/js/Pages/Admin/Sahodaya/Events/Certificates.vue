    ...
    <SahodayaEventsLayout :title="`${event.title} — Certificates`" :sahodaya="sahodaya" :event="event" :publicUrl="publicUrl"
        ...
        <div class="card !p-3 mb-6 text-xs">
            ...
            <datalist id="signatory-labels">
                <option v-for="l in signatoryLabelSuggestions" :key="l" :value="l" />
            </datalist>
...
const signatories = ref(props.certificateSignatories.map(newSignatory));
const signatoriesSaving = ref(false);

watch(() => props.certificateSignatories, (rows) => { signatories.value = rows.map(newSignatory); });
