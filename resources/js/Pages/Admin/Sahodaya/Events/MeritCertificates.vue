<template>
    <SahodayaEventsLayout :title="`${event.title} — Merit Certificates`" :sahodaya="sahodaya" :event="event" :publicUrl="publicUrl"
                         :pendingPaymentsCount="pendingPaymentsCount" :show-header-title="false">
        <PageHeader :title="`${event.title} — Merit Certificates`" eyebrow="Operations"
                    description="Winner certificates (Merit Ranks 1–3), filterable by item or school." />
        ...
        <div class="card p-4">
            ...
            <div v-else class="text-center text-gray-500 text-sm py-8">No merit certificates match this scope yet.</div>
...
function scopeParams() {
    const params = new URLSearchParams({ cert_type: 'winner' });
...
function generate(itemId) {
    router.post(`${base}/generate`, itemId ? { item_id: itemId } : {}, { preserveScroll: true });
}
