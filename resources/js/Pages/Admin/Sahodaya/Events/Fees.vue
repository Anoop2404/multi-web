        ...
        <div v-if="childEvents.length" class="card mb-4 !py-3">
            <div class="flex flex-wrap gap-3 items-center">
                ...
                <SearchableSelect :model-value="String(event.id)" @update:model-value="switchSportEvent"
                                  :options="sportEventSwitcherOptions" :all-option="false"
                                  placeholder="Select event" class="w-64" />
            </div>
        </div>
...
function switchSportEvent(value) {
    router.get(`/sahodaya-admin/${props.sahodaya.id}/events/${value}/fees`);
}

const sportEventSwitcherOptions = computed(() => props.childEvents.map(ev => ({
    value: String(ev.id),
