<template>
    <SahodayaEventsLayout title="Mark entry" :sahodaya="sahodaya" :event="event" :publicUrl="publicUrl"
        ...
        <div class="card !p-4 space-y-3 mb-5">
            ...
            <!-- Bulk print sheets (select several items/phase/area, download combined PDFs)
                 moved to its own page/tab -- see BulkSheets.vue -- since it's a distinct
                 "print for a session" workflow, not something done inline while entering
                 marks for one item at a time. -->
            <div class="pt-3 border-t border-slate-100">
                <Link :href="bulkSheetsUrl" class="text-xs font-semibold text-indigo-700 hover:underline">
                    🖨️ Bulk print sheets for several items →
                </Link>
            </div>
        </div>
...
// Bulk print sheets (multi-item/phase/area combined PDFs) now lives on its own page —
// see BulkSheets.vue — this just links out to it.
const bulkSheetsUrl = computed(() => `/sahodaya-admin/${props.sahodaya.id}/events/${props.event.id}/bulk-sheets`);
