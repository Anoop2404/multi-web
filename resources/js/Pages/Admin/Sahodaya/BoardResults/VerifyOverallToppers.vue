<template>
    <SahodayaAdminLayout :title="pageTitle" :sahodaya="sahodaya" :publicUrl="publicUrl" :show-header-title="false">
        ...
        <BoardResultsVerificationSubNav :sahodayaId="sahodaya.id" active="toppers-overall" :currentClass="selectedClass" />

        <p v-if="selectedClass" class="text-sm -mt-2 mb-4">
            <Link :href="`/sahodaya-admin/${sahodaya.id}/board-results/verification/overall?class=${selectedClass === 12 ? 10 : 12}&status=${filters.status}`" class="text-indigo-600 hover:underline font-medium">
        ...
        <div class="flex flex-wrap gap-2 mb-4">
            <Link v-for="(label, value) in statusOptions" :key="value"
                  :href="`/sahodaya-admin/${sahodaya.id}/board-results/verification/overall?class=${selectedClass}&status=${value}`"
                ...
                <table class="w-full text-left text-sm">
                    <thead class="bg-slate-50 text-slate-700 font-semibold border-b border-slate-200">
                        <tr>
                            ...
                            <th v-if="selectedClass === 12" class="py-3 px-4">Stream</th>
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        <tr v-for="t in toppers.data" :key="t.id" class="hover:bg-slate-50/50">
                            ...
                            <td v-if="selectedClass === 12" class="py-3 px-4 text-xs text-slate-600">
                                {{ t.stream || '—' }}
                            </td>
                            <td class="py-3 px-4 text-center font-bold text-slate-700">
                                {{ t.marks_obtained }} <span class="text-xs font-normal text-slate-400">/ {{ t.total_marks }}</span>
                            </td>
                        ...
                        <tr v-if="!toppers.data.length">
                            <td :colspan="selectedClass === 12 ? 8 : 7" class="text-center py-12 text-slate-400 text-sm">
                                No toppers found for this view.
                            </td>
                        </tr>
                    </tbody>
                </table>
...
const pageTitle = computed(() => `Verify ${props.selectedClass === 12 ? 'Stream' : 'Overall'} Toppers - Class ${props.selectedClass === 12 ? 'XII' : 'X'}`);
