...
class FestReportService
{
    ...
    {
        return $this->event->items()->orderBy('display_order')->get();
    }
    ...
    public function participantsFlat(?int $itemId = null, ?string $classGroup = null, ?string $schoolId = null, ?int $studentId = null, ?int $teacherId = null)
    {
    ...
    {
        return $this->items()->map(function (FestEventItem $item) {
            $approved = FestRegistration::where('event_id', $this->event->id)
                ->where('item_id', $item->id)
                ->where('status', 'approved')
                ->count();
            $pending = FestRegistration::where('event_id', $this->event->id)
                ->where('item_id', $item->id)
    ...
    {
        $regs = $this->approvedRegistrations(
    ...
    private function attendanceSheetSchoolPdf(Request $request): \Illuminate\Http\Response
    {
        ...
        $participants = $this->participantsFlat(null, null, $school->id);
        $studentRows = [];
    ...
    private function judgeSheetPdf(Request $request): \Illuminate\Http\Response
    {
        ...
        $participants = $this->participantsFlat($itemId);
