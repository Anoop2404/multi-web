...
class TenantUserController extends SahodayaAdminController
{
    ...
    public function store(Request $request, TenantUserProvisioner $provisioner, PlatformAuditLogger $audit)
    {
        ...
        $data = $request->validate([
            ...
            'region_admin_event_id'  => [
                Rule::requiredIf(fn () => in_array('region_admin', $request->input('roles', []), true)),
                'nullable',
                'exists:fest_events,id',
            ],
            'region_admin_region_id' => [
                Rule::requiredIf(fn () => in_array('region_admin', $request->input('roles', []), true)),
                'nullable',
     ...
     * @param  array<string, mixed>  $data
     */
    private function syncRegionAdminAssignment(User $user, array $data): void
    {
        if (! in_array('region_admin', $data['roles'] ?? [], true)) {
            return;
        }

        $eventId = $data['region_admin_event_id'] ?? null;
    ...
    private function permissionLabels(): array
    {
        return [
            ...
            'mcq.manage' => 'Talent Search — manage',
            'mcq.attendance' => 'Talent Search — attendance',
            'mcq.marks' => 'Talent Search — marks',
