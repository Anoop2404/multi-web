...
class FestRegistrationController extends SchoolAdminController
{
    ...
    {
        if (! in_array($eventType, ['kalolsavam', 'sports', 'english_fest', 'science_fest', 'kids_fest', 'teacher_fest'], true)) {
            return null;
        }
    ...
    private function hydrateEventForSchoolRegistration(
        ...
        $event->setAttribute('class_group_labels', FestClassGroupScheme::labels(null, $event->rootEvent()));
    ...
    public function uploadEventPayment(Request $request, string $tenantId, FestEvent $event, string $program)
    {
        ...
        // Fields are validated as nullable above and may be entirely absent from $data when
        // not submitted (Laravel's validate() only includes keys present in the request) —
        // use ?? null rather than direct array access so a missing key doesn't throw, and
        // pass amount through as null (not cast to 0.0) so the service's own outstanding-
        // balance default applies.
